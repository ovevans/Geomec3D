...
while step < numOfSteps:
	...
	t3.append(float(reader3.getTimeLevel(step))/3.6e3)
	...
	p3.append(float(reader3.getSolutionAtNodeAndStep("p", reader3.getNode(Point(0., 0., 0.)), step))/1e3)
	step += 6
tmin = min(t1)
tmax = max(t1)
...
plt.plot(t3, p3, mark3, fillstyle="none", label=label3, ms=6, mec="k", mew=0.75)
