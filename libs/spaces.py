...
class DisplacementPressureVolStressSpace(object):
	def __init__(self, grid, u_degree, p_degree, sig_degree):
		v_elem = VectorElement("CG", grid.mesh.ufl_cell(), u_degree)
		q_elem = FiniteElement("CG", grid.mesh.ufl_cell(), p_degree)
		tau_elem = FiniteElement("CG", grid.mesh.ufl_cell(), sig_degree)
		self.V = FunctionSpace(grid.mesh, v_elem)
		self.Q = FunctionSpace(grid.mesh, q_elem)
		self.T = FunctionSpace(grid.mesh, tau_elem)
		self.mixedSpace = BlockFunctionSpace([self.V, self.Q, self.T])
